...
describe('Notificatinos component test cases', () => {
  ...
  it('verifies component does not re-render with same list', () => {
    ...
    const nextProps = { listNotifications: listNotifications, displayDrawer: false };
  ...
  it('calls handleDisplayDrawer when clicking on the menu item', () => {
    ...
    const wrapper = shallow(<Notifications listNotifications={listNotifications} displayDrawer={false} handleDisplayDrawer={handleDisplayDrawer} />);
    wrapper.find('div div').simulate('click'); // second div with 'Your notifications' text
    expect(handleDisplayDrawer).toHaveBeenCalledTimes(1);
  });

  it('calls handleHideDrawer when clicking on the button', () => {
    const handleHideDrawer = jest.fn();
    const wrapper = shallow(<Notifications listNotifications={listNotifications} displayDrawer={true} handleHideDrawer={handleHideDrawer} />);
